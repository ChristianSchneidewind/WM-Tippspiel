...
public class KnockoutBracketService
{
    ...
    private string? ResolveMatchOutcome(
        ...
        IReadOnlyDictionary<int, GroupStanding> thirdPlaceAssignments)
    {
        ...
        // 1. Logik für ein normales Spielergebnis (Entscheidung ohne Elfmeterschießen)
        if (sourceGame.HomeTeamScore != sourceGame.AwayTeamScore)
        {
            var homeWonNormal = sourceGame.HomeTeamScore > sourceGame.AwayTeamScore;
            return winner
                ? (homeWonNormal ? homeName : awayName)
                : (homeWonNormal ? awayName : homeName);
        }
        ...
        // Kosmetischer Platzhalter: Das Spiel steht Unentschieden (z.B. im Live-Ticker), 
        // aber das Elfmeterschießen läuft noch oder wurde noch nicht eingetragen.
        return $"Sieger Spiel #{sourceMatchNumber}";
    }
